...
for (each.tissue in Tissues) {
  ...
  File.Name <- paste("221_GTEx_Exons_Constitutive_vs_Alternative/221_Scatter_Plot_ConstitutiveVsAlternative_",
                     tissue.no.spaces,
                     ".pdf",
                     sep = "",
                     collapse = "")
...
My.Plot <- ggplot(data = Combined.Plot.DF,
  ...
  annotate(geom = "text",
           ...
           hjust = 0,
           vjust = 0)
ggsave(filename = "221_Pooled_Data_Exons_ConstitutiveVsAlternative.pdf", plot = My.Plot, width = 12, height = 12, useDingbats = F)
