...
for (n in seq(0.50,0.99,0.01)) {
  ...
  for (each.tissue in Tissues) {
    ...
    Plot.DF <- data.frame(Exon.ID = Exon.IDs,
                          Mutation.Effect = Mutation.Effects * 100 * 2, # fix this when I get proper double slope data
...
save(Parameter.DF, file = "229_Simultaneously_Change_Constitutive_and_Alternative_Exon_Threshold.RData")

load("229_Simultaneously_Change_Constitutive_and_Alternative_Exon_Threshold.RData")
...
ggsave(filename = "229_Simultaneous_Parameter_Shift.pdf", plot = My.Plot, width = 8, height = 8, useDingbats = F)
...
for (n in seq(0.60,0.99,0.01)) {
  ...
  for (m in seq(0.60,0.99,0.01)) {
    ...
    for (each.tissue in Tissues) {
      ...
      Plot.DF <- data.frame(Exon.ID = Exon.IDs,
                            Mutation.Effect = Mutation.Effects * 100 * 2, # until i get real DoubleSlope
                            Starting.PSI = Starting.PSIs * 100,
                            Exon.Class = Constitutive.Alternative.Exon.DF[Exon.IDs,"Exon.Class"])
...
save(Parameter.DF, file = "229_Independently_Change_Constitutive_and_Alternative_Exon_Threshold.RData")
...
Parameter.DF$Loess.Smoother[which(Parameter.DF$Loess.Smoother < 4)] <- 4
My.Plot <- ggplot(data = Parameter.DF,
       mapping = aes(x = Constitutive.Exon.Threshold,
                     y = Alternative.Exon.Threshold,
                     z = Loess.Smoother)) +
...
ggsave(filename = "229_2D_Parameter_Plot.pdf", plot = My.Plot, width = 8, height = 8)
